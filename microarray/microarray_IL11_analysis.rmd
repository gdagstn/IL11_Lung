...
Smoker information is present only in a subset of samples. We subset the targets and run the same analysis:
...
*IL11* is the most negatively correlated with DL<sub>CO</sub> % predicted, which is the most important index for IPF severity. A low DLCO means high disease severity, so a high negative correlation means that there is an inverse relationship with lung function and *IL11* expression. Other lung function measurements are correlated, but we must remember that the difference in FEV1 is not statistically significant between healthy individuals and IPF patients. 
